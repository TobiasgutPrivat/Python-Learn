...
class Sudoku:
    ...
    def __init__(self, size: int = 9, board: list[list[int|None]] | list[list[Cell]] = None, doLog: bool = False):
        ...
        if board is None:
            ...
            self.board = [[Cell([board[row][col]], col, row, logger) if board[row][col] and int(board[row][col]) in self.values else Cell(self.values.copy(), col, row, logger) for col in range(size)] for row in range(size)]
        ...
        for row in self.board:
            for cell in row:
                for i in self.values:
                    if cell.getValue() is None and i in cell.possibleValues:
                        newBoard = self.getList()
